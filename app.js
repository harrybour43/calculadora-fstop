...
function abrirModal(dataISO, ucNome, profNome, numAula) {
  document.getElementById("sheet-kicker").innerText = `${getDiaDaSemana(
    dataISO
  ...
  document.getElementById(
    "sheet-subtitle"
  ).innerText = `Aula ${numAula} • Prof. ${profNome}`;
  ...
  if (eventosDoDia.length > 0) {
    ...
    bodyHTML =
      "<em>Nenhum material extra ou evento cadastrado para esta data.</em>";
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (versionDisplay)
    versionDisplay.innerText = `v${APP_VERSION} • Harry Bour • 2026`;

  const btnSyncMenu = document.getElementById("btn-sync-menu");
  if (btnSyncMenu)
    btnSyncMenu.innerHTML = `🔄 Sincronizar Dados (v${APP_VERSION})`;
  ...
  if (localStorage.getItem("tpf43m_aulas"))
    processarAulas(localStorage.getItem("tpf43m_aulas"));
  if (localStorage.getItem("tpf43m_eventos"))
    processarEventos(localStorage.getItem("tpf43m_eventos"));
  if (localStorage.getItem("tpf43m_datas"))
    processarDatas(localStorage.getItem("tpf43m_datas"));
  if (localStorage.getItem("tpf43m_dicio"))
    processarDicio(localStorage.getItem("tpf43m_dicio"));
...
async function fetchGoogleSheets() {
  ...
  async function tryFetch(url, storageKey, processFunc) {
    try {
      ...
      console.warn(
        `Aviso: Não foi possível sincronizar ${storageKey} no modo offline.`
      );
    }
  }
  ...
  Promise.all([
    ...
    tryFetch(
      LINK_PLANILHA_EVENTOS + noCacheStr,
      "tpf43m_eventos",
      processarEventos
    ),
...
function processarDataInteligente(dataRaw) {
  ...
  return {
    ...
    mes: parseInt(mes),
    dia: parseInt(dia),
    iso: `${ano}-${mes.toString().padStart(2, "0")}-${dia
      .toString()
      .padStart(2, "0")}`,
  };
}
...
document.getElementById("filtroUC").addEventListener("change", function (e) {
  ...
  dUC.forEach(function (a) {
    ...
    if (isPast) {
      rowStyle =
        "padding:12px 0; border-bottom:1px solid var(--border-color); color:var(--text-muted); opacity:0.6;";
    }

    htmlHTML += `<div style="${rowStyle}" onclick="abrirModal('${a.data}','${
      a.uc
    }','${a.docente}','${a.aula}')">
                        <strong style="color:#fff;">${formatarDataBR(
                          a.data
                        )}</strong> - Prof. ${a.docente} 
                        <span style="display:block; font-size:0.85em; color:var(--text-muted);">Aula ${
                          a.aula
                        }</span>
                      </div>`;
  });
...
function mudarSemana(offset) {
  ...
  const sextaFeira = new Date(
    segundaFeira.getFullYear(),
    segundaFeira.getMonth(),
    segundaFeira.getDate() + 4,
    12,
    0,
    0
  );
  ...
  aSemana.forEach(function (a) {
    ...
    htmlHTML += `<div class="card" style="border-top-width:4px; border-top-color:${colorHex};" onclick="abrirModal('${
      a.data
    }', '${a.uc}', '${a.docente}', '${a.aula}')">
                            ...
                            <div style="font-size:1.8rem; font-family:'Cormorant Garamond', serif; font-weight:700; margin-bottom:8px; color:#fff;">${formatarDataBR(
                              a.data
                            )}</div>
                            <div style="color:var(--text-muted); font-size:0.95rem; line-height:1.4;">
                                <strong style="color:#fff;">${
                                  a.uc
                                }</strong><br>Prof: ${a.docente}
                            </div>
                        </div>`;
  });
...
function initEventos() {
  ...
  if (baseEventos.length === 0) {
    container.innerHTML =
      "<p style='text-align:center; color:var(--text-muted);'>Nenhum evento interno programado.</p>";
    return;
  }
  ...
  baseEventos.forEach(function (ev) {
    ...
    htmlHTML += `<div class="card" style="border-top-width:4px; border-top-color:var(--accent-eventos);">
      ...
      .padStart(2, "0")}</span>
                            ...
                            <div class="ev-desc" style="margin-bottom:0;">${
                              ev.desc
                            }</div>
                        </div>`;
    eventosExibidos++;
  ...
  if (eventosExibidos === 0) {
    htmlHTML =
      "<p style='text-align:center; color:var(--text-muted);'>Nenhum evento recente ou futuro.</p>";
  }
...
function processarDatas(tsv) {
  ...
  linhas.forEach(function (colunas) {
    if (colunas.length >= 1 && colunas[0] && colunas[0].trim() !== "") {
      ...
      const dtFim =
        colunas.length > 1 && colunas[1].trim() !== ""
          ? processarDataInteligente(colunas[1].trim())
          : dtInic;
      const categoriaRaw =
        colunas.length > 2 && colunas[2].trim() !== ""
          ? colunas[2].trim()
          : "Evento";
...
function renderFiltrosAgenda() {
  ...
  baseDatas.forEach(function (d) {
    d.categorias.forEach(function (catObj) {
      setCat.add(catObj.nome);
    });
    if (d.uf) {
      setUF.add(d.uf);
    }
    ...
    const dtInicObj = new Date(
      d.dtInic.ano,
      d.dtInic.mes - 1,
      d.dtInic.dia
    ).getTime();
    const dtFimObj = new Date(
      d.dtFim.ano,
      d.dtFim.mes - 1,
      d.dtFim.dia
    ).getTime();

    const diffInicDays = Math.round(
      (dtInicObj - hojeTime) / (1000 * 60 * 60 * 24)
    );
    const diffFimDays = Math.round(
      (dtFimObj - hojeTime) / (1000 * 60 * 60 * 24)
    );
  ...
  Array.from(setCat)
    ...
    .forEach(function (c) {
      ...
      htmlCat += `<div class="filter-chip ${cor} ${isActive}" onclick="toggleFiltroCategoria('${c.replace(
        /'/g,
        "\\'"
      )}')">${c}</div>`;
    });
...
function initDatas() {
  ...
  if (baseDatas.length === 0) {
    container.innerHTML =
      "<p style='text-align:center; color:var(--text-muted);'>Nenhuma data cadastrada na base.</p>";
    return;
  }
  ...
  baseDatas.forEach(function (ev) {
    ...
    const temCategoriaAtiva =
      filtrosCatAtivos.size === 0 ||
      ev.categorias.some(function (cat) {
        return filtrosCatAtivos.has(cat.nome);
      });

    if (!temCategoriaAtiva) return;
    if (filtrosUFAtivos.size > 0 && (!ev.uf || !filtrosUFAtivos.has(ev.uf)))
      return;
    ...
    const isUltimosDias =
      diffInicDays <= 0 && diffFimDays >= 0 && diffFimDays <= 7;
    ...
    if (ev.dtFim && ev.dtFim.iso !== ev.dtInic.iso) {
      periodoHTML = `De ${formatarDataBR(ev.dtInic.iso)} a ${formatarDataBR(
        ev.dtFim.iso
      )}/${ev.dtFim.ano}`;
    } else {
    ...
    const classeBordaPrincipal =
      ev.categorias.length > 0 ? ev.categorias[0].classe : "cat-default";
    ...
    htmlHTML += `
                    <div class="card datas-int ${classeBordaPrincipal}">
                        ...
                        <div style="display: flex; gap: 8px; margin-top: 24px; flex-wrap: wrap;">
                            ${
                              ev.link
                                ? `<a href="${ev.link}" target="_blank" class="btn btn-outline" style="font-size:0.8rem; padding:10px;">SAIBA MAIS (Link externo)</a>`
                                : ""
                            }
  ...
  if (datasExibidas === 0) {
    htmlHTML =
      "<p style='text-align:center; color:var(--text-muted);'>Nenhum evento encontrado com os filtros selecionados.</p>";
  }
  container.innerHTML = htmlHTML;
}
...
function renderDicioSearch() {
  ...
  if (dicioArray.length === 0) {
    container.innerHTML =
      "<p style='text-align:center; color:var(--text-muted); grid-column: 1/-1;'>Nenhum dado vinculado ainda.</p>";
    return;
  }
  ...
  const itensFiltrados = dicioArray
    .filter(function (item) {
      const matchTexto =
        item.termo.toLowerCase().includes(inputFiltro) ||
        item.definicao.toLowerCase().includes(inputFiltro);
  ...
  if (itensFiltrados.length === 0) {
    container.innerHTML =
      "<p style='text-align:center; color:var(--text-muted); grid-column: 1/-1;'>Nenhum termo encontrado.</p>";
    return;
  }
...
function getEducativeInfo(time, lat, lng) {
  ...
  let altText = "";
  if (altDeg < 0) {
    altText = "Abaixo do horizonte (Luz indireta)";
  } else if (altDeg < 15) {
    altText = "Muito baixo (Sombras longas)";
  } else if (altDeg < 45) {
    altText = "Baixo (Sombras médias)";
  } else if (altDeg < 75) {
    altText = "Alto (Sombras curtas)";
  } else {
    altText = "A pino (Sombras duras)";
  }

  let dir = "";
  if (aziDeg >= 337.5 || aziDeg < 22.5) {
    dir = "Norte (N)";
  } else if (aziDeg < 67.5) {
    dir = "Nordeste (NE)";
  } else if (aziDeg < 112.5) {
    dir = "Leste (L)";
  } else if (aziDeg < 157.5) {
    dir = "Sudeste (SE)";
  } else if (aziDeg < 202.5) {
    dir = "Sul (S)";
  } else if (aziDeg < 247.5) {
    dir = "Sudoeste (SO)";
  } else if (aziDeg < 292.5) {
    dir = "Oeste (O)";
  } else {
    dir = "Noroeste (NO)";
  }

  return `<div class="info">📐 <strong>Altura (${Math.round(
    altDeg
  )}°):</strong> ${altText}<br>🧭 <strong>Direção:</strong> A luz vem do ${dir}</div>`;
}
...
function getMoonInfo(targetDate) {
  ...
  if (phase < 0.03 || phase > 0.97) {
    phaseName = "Nova";
    emoji = "🌑";
    witticism = "Zero luz! Paraíso das astrofotografias.";
  } else if (phase < 0.25) {
    phaseName = "Crescente";
    emoji = "🌒";
    witticism = "Rende fotos com luz suave noturna.";
  } else if (phase < 0.28) {
    phaseName = "Quarto Crescente";
    emoji = "🌓";
    witticism = "Luz lateral dramática, ótima para crateras.";
  } else if (phase < 0.47) {
    phaseName = "Gibosa Crescente";
    emoji = "🌔";
    witticism = "Quase cheia! A luz já ofusca estrelas.";
  } else if (phase < 0.53) {
    phaseName = "Cheia";
    emoji = "🌕";
    witticism = "Um rebatedor gigante no céu. Luz dura.";
  } else if (phase < 0.72) {
    phaseName = "Gibosa Minguante";
    emoji = "🌖";
    witticism = "A iluminação começa a diminuir.";
  } else if (phase < 0.78) {
    phaseName = "Quarto Minguante";
    emoji = "🌗";
    witticism = "Ideal para captar o nascer da lua de madrugada.";
  } else {
    phaseName = "Minguante";
    emoji = "🌘";
    witticism = "Brilho sumindo, como a bateria da câmera.";
  }

  let supermoonTag = "";
  if (distance < 365000 && phase > 0.45 && phase < 0.55) {
    supermoonTag =
      "<br><br>🌕 <strong>SUPER LUA:</strong> Perigeu. Prepare a teleobjetiva!";
  ...
  return `
                <div class="card sun-card moon-card">
                    <div class="row">
                        <span class="label">Fase da Lua (${Math.round(
                          fraction * 100
                        )}%)</span>
...
async function updateWeatherUI(targetDate) {
  ...
  weatherDiv.innerHTML =
    '<span style="color:var(--text-muted);">🌤️ Consultando meteorologia...</span>';
  ...
  if (!weather) {
    weatherDiv.innerHTML =
      '<span style="color:var(--text-muted);">⚠️ Clima indisponível no modo offline.</span>';
    return;
  }
...
function renderSunCalc(targetDate) {
  ...
  document.getElementById(
    "local-ref-title"
  ).innerHTML = `📍 Referência: <strong>${locName}</strong>`;
  ...
  let htmlHTML = `
                ...
                <div class="card sun-card golden">
                    <div class="row">
                        ...
                        <span class="time">${formatTime(
                          times.sunriseEnd
                        )} - ${formatTime(times.goldenHourEnd)}</span>
                    </div>
                ...
                <div class="card sun-card golden">
                    <div class="row">
                        ...
                        <span class="time">${formatTime(
                          times.goldenHour
                        )} - ${formatTime(times.sunsetStart)}</span>
                    </div>
  ...
  document.getElementById("meteor-results").innerHTML =
    getMeteorInfo(targetDate); // Motor de Meteoros
...
function getMeteorInfo(targetDate) {
  ...
  const chuvas = [
    {
      ...
      pico: { m: 1, d: 3 },
      taxa: 120,
      descricao: "Forte, mas janela de pico muito curta.",
    },
    {
      nome: "Líridas",
      inicio: { m: 4, d: 14 },
      ...
      pico: { m: 4, d: 22 },
      taxa: 18,
      descricao: "Meteoros brilhantes, boa para fotografia.",
    },
    {
      nome: "Eta Aquáridas",
      inicio: { m: 4, d: 19 },
      ...
      pico: { m: 5, d: 6 },
      taxa: 50,
      descricao: "Originada pelo Cometa Halley, excelente no Hemisfério Sul.",
    },
    {
      nome: "Delta Aquáridas",
      inicio: { m: 7, d: 12 },
      fim: { m: 8, d: 23 },
      pico: { m: 7, d: 30 },
      taxa: 25,
      descricao: "Dois radiantes, ritmo constante e prolongado.",
    },
    {
      nome: "Perseidas",
      inicio: { m: 7, d: 17 },
      ...
      taxa: 100,
      descricao:
        "A mais famosa do ano. Muitos meteoros brilhantes e rastros longos.",
    },
    {
      nome: "Orionídeas",
      inicio: { m: 10, d: 2 },
      ...
      pico: { m: 10, d: 21 },
      taxa: 20,
      descricao: "Meteoros muito rápidos, também poeira do Cometa Halley.",
    },
    {
      nome: "Leônidas",
      inicio: { m: 11, d: 6 },
      ...
      pico: { m: 11, d: 17 },
      taxa: 15,
      descricao: "Famosa por gerar 'tempestades' históricas no passado.",
    },
    {
      nome: "Geminídeas",
      inicio: { m: 12, d: 4 },
      ...
      pico: { m: 12, d: 14 },
      taxa: 150,
      descricao: "A melhor chuva do ano. Intensa, multicolorida e confiável.",
    },
    {
      nome: "Ursídeas",
      inicio: { m: 12, d: 17 },
      ...
      pico: { m: 12, d: 22 },
      taxa: 10,
      descricao: "Baixa intensidade, estritamente visível nas madrugadas.",
    },
  ];
  ...
  let badge = isPico
    ? `<span style="background: var(--accent-eventos); color: #fff; font-size: 0.6rem; font-weight: 800; font-family: 'DM Sans', sans-serif; padding: 4px 6px; border-radius: 4px; vertical-align: middle; margin-left: 8px; letter-spacing: 1px;">NOITE DE PICO!</span>`
    : "";
...
async function buscarLocalidadeSunCalc() {
  ...
  try {
    ...
    alert("Erro de conexão ao buscar localidade.");
  } finally {
...
function getMyLocation() {
  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(
      ...
      function (error) {
        alert(
          "Erro ao obter GPS. Verifique as permissões de localização do seu navegador."
        );
        btn.innerText = btnOriginalText;
        btn.disabled = false;
      },
...
function addLight(role = "Preenchimento", baseStop = 6, tenths = 0) {
  if (lights.length >= 6) {
    alert(
      "Capacidade máxima do estúdio atingida: 1 Principal + 5 Secundárias."
    );
    return;
  }
...
function renderLights() {
  ...
  lights.forEach(function (light) {
    ...
    let optionsRole = `
                    <option ${
                      light.role === "Principal" ? "selected" : ""
                    }>Principal</option>
                    <option ${
                      light.role === "Preenchimento" ? "selected" : ""
                    }>Preenchimento</option>
                    <option ${
                      light.role === "Recorte" ? "selected" : ""
                    }>Recorte</option>
                    <option ${
                      light.role === "Cabelo" ? "selected" : ""
                    }>Cabelo</option>
                    <option ${
                      light.role === "Fundo" ? "selected" : ""
                    }>Fundo</option>
                    <option ${
                      light.role === "Retorno" ? "selected" : ""
                    }>Retorno</option>
                `;
    ...
    if (isPrincipal) {
      ...
      const colorFeedback = advice.isWarning
        ? "var(--accent-warning)"
        : "var(--text-main)";
...
async function solicitarPermissaoNotificacao() {
  ...
  if (permissao === "granted") {
    ...
    alert(
      "Para receber alertas, você precisa autorizar as notificações nas configurações do seu celular ou navegador."
    );
  }
}
  ...
  if (installBtn) {
    installBtn.addEventListener("click", async () => {
      if (deferredPrompt) {
        ...
        alert(
          "Para instalar no iOS: toque no botão de 'Compartilhar' (quadrado com seta para cima) e depois em 'Adicionar à Tela de Início'."
        );
      }
    });
  }
...
// ==========================================
// MÓDULO SISTEMA DE ZONAS (Fotometria e Hiperfocal)
// ==========================================
...
document
  .getElementById("focal-length")
  .addEventListener("input", calculateZonasHyperfocal);
document
  .getElementById("sensor-coc")
  .addEventListener("change", calculateZonasHyperfocal);
...
function calculateZonasHyperfocal() {
  ...
  if (f > 0 && c > 0) {
    // H = (f^2) / (N * c). Divisão por 1000 para converter mm em Metros.
    const h8 = (f * f) / (8 * c) / 1000;
    const h11 = (f * f) / (11 * c) / 1000;

    let html = `<strong>Sugestão de Abertura (Sweet Spots):</strong><br>`;
    html += `Em <strong>f/8</strong>: Foque em <strong>${h8.toFixed(
      1
    )} metros</strong>.<br>`;
    html += `Em <strong>f/11</strong>: Foque em <strong>${h11.toFixed(
      1
    )} metros</strong>.<br><br>`;
    html += `<span style="font-size: 0.75rem; color: var(--safe);">Focando nesta distância, a nitidez vai da metade desse valor até o infinito.</span>`;
...
// Chamada inicial
calculateZonasHyperfocal();

function parseZonasSpeed(str) {
...
// ==========================================
// FUNÇÃO DE ADICIONAR LINHAS (Movida para o escopo global)
// ==========================================
let zonasRowCount = 5;
const maxZonasRows = 12;

function addZonaRow() {
  if (zonasRowCount >= maxZonasRows) return;

  zonasRowCount++;
  const container = document.getElementById("zonas-inputs-container");
  const div = document.createElement("div");
  div.style.cssText = "display: flex; gap: 10px;";
  div.innerHTML = `
                <input type="text" class="input-box input-name-zona" placeholder="Ponto ${zonasRowCount}">
                <input type="text" class="input-box input-speed-zona" placeholder="" style="width: 90px; text-align: center;">
            `;
  container.appendChild(div);

  // Oculta o botão ao atingir 12 para manter a UI limpa
  if (zonasRowCount >= maxZonasRows) {
    document.getElementById("btn-add-zona").style.display = "none";
  }
}

function calculateZonasExposure() {
  const inputsSpeed = document.querySelectorAll(".input-speed-zona");
  const inputsName = document.querySelectorAll(".input-name-zona");
  let times = [];
  let maxT = -Infinity; // Mais longo (escuro)
  let minT = Infinity; // Mais rápido (claro)

  inputsSpeed.forEach((input, index) => {
    if (input.value) {
      const t = parseZonasSpeed(input.value);
      if (!isNaN(t) && t > 0) {
        times.push({ name: inputsName[index].value || `P${index + 1}`, t: t });
        if (t > maxT) maxT = t;
        if (t < minT) minT = t;
      }
    }
